...
-- Modern nvim-cmp configuration with performance optimizations
local M = {}
...
function M.setup()
    ...
    local source_names = {
        nvim_lsp = "LSP", luasnip = "Snp", buffer = "Buf",
        path = "Path", cmdline = "Cmd", crates = "Crate", git = "Git",
    }
                    ...
                    for _, b in ipairs(vim.api.nvim_list_bufs()) do
                        if vim.api.nvim_buf_is_valid(b) and vim.api.nvim_buf_get_option(b, "buftype") == "" then
                            ...
                            if fname == "" then
                                ...
                                local stat = safe_fs_stat(fname)
                                if stat and stat.size and stat.size < max_size then
                                    bufs[b] = true
                                end
                            end
                        end
    ...
    cmp.setup({
        ...
        performance = {
            debounce = 30, throttle = 20, fetching_timeout = 100,
            max_view_entries = 25, async_budget = 2, max_chunk_size = 1000,
        },
        ...
        window = {
            completion = cmp.config.window.bordered({
                ...
                col_offset = -3, side_padding = 1,
            }),
            documentation = cmp.config.window.bordered({
                winhighlight = "Normal:CmpDocNormal,FloatBorder:CmpDocBorder",
                max_width = 60, max_height = 15,
            }),
        },
        formatting = {
            ...
            format = function(entry, item)
                ...
                item.menu = source_names[entry.source and entry.source.name or ""] or "[" .. (entry.source and entry.source.name or "") .. "]"
                if #item.abbr > 48 then item.abbr = item.abbr:sub(1, 45) .. "…" end
                return item
            end,
        },
        ...
        sorting = {
            ...
            comparators = {
                ...
                cmp.config.compare.offset, cmp.config.compare.kind,
        ...
        sources = (function()
            ...
            local stat = safe_fs_stat(vim.api.nvim_buf_get_name(0))
            local file_size = stat and stat.size or nil
    ...
    if safe_require("crates") then
        cmp.setup.filetype({ "rust", "toml" }, {
            sources = cmp.config.sources({
                { name = "nvim_lsp" }, { name = "crates" }, { name = "luasnip" },
            }, { { name = "buffer" } }),
        })
    end
    ...
    vim.api.nvim_create_user_command("CmpStatus", function()
        ...
        print(vim.inspect({
            ...
            sources = vim.tbl_map(function(s) return s.name end, cfg.sources or {}),
        }))
    end, { desc = "Show cmp status" })
